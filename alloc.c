...
static __inline__ void *GGGGC_malloc_inline(size_t sz, unsigned short ptrs, int init)
{
    void *ret;
...
{
    return GGGGC_malloc_inline(sz, ptrs, init);
}

void *GGGGC_malloc_ptr_array(size_t sz, int init)
{
    return GGGGC_malloc_inline(sizeof(struct GGGGC_Header) + sz * sizeof(void *), sz, init);
}
